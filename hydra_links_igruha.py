...
def make_magnet_from_bytes(torrent_bytes):
    try:
        ...
        # print(format_size(total_length))
        ...
        if b'creation date' in metadata:
            ...
            # Конвертуємо Unix timestamp в зручний формат
        ...
        # return magnet_link, format_size(total_length), formatted_date
        ...
        if navbartor:
            ...
            if center_tag:
                ...
                if size_and_info and download_page_link:
                    ...
                    size, name_details = extract_size_and_info(size_and_info.get_text(strip=True))
                    ...
                    try:
                        ...
                        magnet_link, t_date = make_magnet_from_bytes(torrent_bytes.getvalue())

                    except requests.RequestException as e:
                        print(f"Не вдалося завантажити {torrent_url}: {e}")
                        magnet_link, t_date = None, None  # Встановлюємо значення None, якщо не вдалося завантажити

                    if magnet_link:
                        # Додаємо інформацію про торрент до списку

                        # torrent_info_list.append({
                        #     'link': torrent['href'],  # Силка на торрент
                        #     'info': name_details,  # Текст розміру
                        #     'size': size,  # Текст розміру
                            
                        #     'date': t_date,  # Дата створення
                            
                        #     'torrent_link': torrent_url,  # Силка на торрент
...
for index, url in enumerate(urls, start=1):
    ...
    date_time, game_name = extract_info_from_page(url)

    if not date_time or not game_name:
        print(f'{index}. INVALID PAGE: {url}')  
    else:
        print(f'{index}. {date_time} | {game_name} | {url}')
        ...
        if (not download_options):
            ...
            for download_option in download_options:
                ...
                title = f"{game_name} {download_option['info']}"

                print(f'    {title} ({download_option['size']})')

                if (download_option['date']):
                    t_date = download_option['date']
                else:
                    t_date = date_time
...
# url = "https://itorrents-igruha.org/2564-train-station-simulator.html"
# torrent_data = get_download_options(url)
